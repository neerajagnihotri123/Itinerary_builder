...
agent_communication:
  - agent: "main"
    message: "🚀 FRONTEND UI ACTION HANDLERS UPDATE: Starting implementation of missing UI action handlers. Backend LLM integration is fully working and generates proper itinerary_display, trip_planner_card, and question_chip actions. Need to fix frontend handlers to properly connect these backend actions to frontend rendering. Key issues identified: 1) itinerary_display sets 'itinerary' state but rendering uses 'generatedItinerary' 2) trip_planner_card shows generic modal instead of using payload 3) Map not connected to conversation context 4) Chat history needs implementation."