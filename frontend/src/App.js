            ...
            <div className="flex-1 overflow-y-auto">
              ...
              <div className="p-6">
                ...
                <div className="relative h-48 rounded-2xl overflow-hidden mb-6 group">
                  {(() => {
                    // Enhanced gallery with multiple images per tour
                    ...
                    return (
                      <>
                        ...
                        {/* View All Button */}
                        <button
                          onClick={() => handleViewAllImages(selectedTour, images)}
                          className="absolute bottom-4 right-4 glass-morphism rounded-lg px-3 py-2 text-white text-sm font-medium hover:bg-white/20 transition-all duration-200 flex items-center gap-2"
                        >
                          <Image className="w-4 h-4" />
                          View All ({images.length})
                ...
                <div className="flex gap-3">
                  <button 
                    onClick={() => handlePlanTour(selectedTour)}
                    className="flex-1 bg-gradient-to-r from-green-600 to-orange-600 text-white font-semibold py-3 px-6 rounded-xl hover:from-green-700 hover:to-orange-700 transition-all duration-200 flex items-center justify-center gap-2"
                  >
                    <Calendar className="w-5 h-5" />
                    Plan Tour - {selectedTour.price}
                  </button>
                ...
                <div className="relative h-48 rounded-2xl overflow-hidden mb-6 group">
                  {(() => {
                    ...
                    return (
                      <>
                        <img
                          src={images[currentActivityImageIndex]}
                        ...
                        {images.length > 1 && (
                          <>
                            <button
                              onClick={() => setCurrentActivityImageIndex(prev => prev === 0 ? images.length - 1 : prev - 1)}
                              className="absolute left-4 top-1/2 transform -translate-y-1/2 bg-black/50 text-white p-2 rounded-full hover:bg-black/70 transition-colors duration-200"
                            >
                              <ArrowLeft className="w-4 h-4" />
                            </button>
                            <button
                              onClick={() => setCurrentActivityImageIndex(prev => prev === images.length - 1 ? 0 : prev + 1)}
                              className="absolute right-4 top-1/2 transform -translate-y-1/2 bg-black/50 text-white p-2 rounded-full hover:bg-black/70 transition-colors duration-200"
                            >
                              <ArrowLeft className="w-4 h-4 rotate-180" />
                            </button>
                        ...
                        {images.length > 1 && (
                          <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex gap-2">
                            {images.map((_, index) => (
                              <button
                                key={index}
                                onClick={() => setCurrentActivityImageIndex(index)}
                                className={`w-3 h-3 rounded-full transition-all duration-200 ${
                                  index === currentActivityImageIndex ? 'bg-white scale-125' : 'bg-white/60 hover:bg-white/80'
                                }`}
                              />
                            ))}
                          </div>
